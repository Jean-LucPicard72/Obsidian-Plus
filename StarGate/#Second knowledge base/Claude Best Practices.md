...
1. [[#Что такое Claude Code и чем он отличается от claude.ai]]
2. [[#Главное ограничение — контекстное окно]]
...
11. [[#Параллельные сессии и автоматизация]]
12. [[#Claude Agent SDK]]
13. [[#Частые ошибки]]
...
## Что такое Claude Code и чем он отличается от claude.ai

Прежде чем начать — важно понять, о чём вообще речь.

### claude.ai — это веб-чат

Заходишь в браузер, общаешься с Claude как в мессенджере. Можно прикрепить файл, получить ответ. **Доступа к твоему компьютеру нет.** Это обычный чат-интерфейс, как ChatGPT.

### Claude Code — это инструмент разработчика

Claude Code работает **в терминале** (командной строке). Ты запускаешь его внутри своего проекта, и он:

- читает твои файлы напрямую
- выполняет команды (`git`, `npm`, `python`, что угодно)
- редактирует код
- запускает тесты
- работает автономно пока ты занимаешься другим
...
| Платформа | Где запускается | Доступ к файлам | Для кого |
|---|---|---|---|
| **claude.ai** | Браузер | Нет | Все, общение и вопросы |
| **Claude Code CLI** | Терминал (`claude`) | Да, твой проект | Разработчики |
| **Claude Code Desktop** | Mac/Windows приложение | Да, с GUI | Разработчики, удобнее CLI |
| **Claude Code в браузере** | Браузер (code.claude.com) | В изолированной VM Anthropic | Когда нет доступа к терминалу |
| **VS Code / JetBrains** | Внутри IDE | Да, тот же Claude Code | Разработчики в редакторе |
...
```
claude.ai   → ты описываешь код словами, Claude отвечает текстом
Claude Code → Claude видит реальные файлы, сам меняет код, сам проверяет
```

Всё руководство ниже — про **Claude Code**, а не про чат.
