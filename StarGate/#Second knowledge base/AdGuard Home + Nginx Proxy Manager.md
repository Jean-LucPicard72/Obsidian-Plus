...
## Статические IP — требования

Любой LXC, на который что-то ссылается, **должен** иметь статический IP:

| Сервис | IP | Почему статический |
|---|---|---|
| AdGuard Home | `192.168.1.53` | DNS-сервер роутера |
| Nginx Proxy Manager | `192.168.1.80` | DNS rewrites, точка входа |
| Tailscale + Mihomo | `192.168.1.163` | NPM форвардит на него, прокси для LXC |
| Proxmox | `192.168.1.104` | NPM форвардит на него |

Статический IP ставится в Proxmox: **CT → Network → eth0 → Edit** (IPv4 + Gateway).
...
> **Обязательно указать Gateway** — без него контейнер не имеет доступа в интернет (`Network is unreachable`). Это касается **каждого** LXC-контейнера которому нужен интернет.

> **RKN:** если роутер уже роутит трафик через Mihomo — все LXC через него тоже идут bypass автоматически. Если нет — перед установкой в контейнере прописать прокси:
> ```bash
